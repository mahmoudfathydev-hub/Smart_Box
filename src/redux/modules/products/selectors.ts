...
export const selectProductDetailsError = (state: RootState) => state.products.productDetailsError;
export const selectRelatedProductsLoading = (state: RootState) =>
  state.products.relatedProductsLoading;
export const selectRelatedProductsError = (state: RootState) => state.products.relatedProductsError;

// Computed selectors
export const selectProductsCount = createSelector([selectProducts], (products) => products.length);

export const selectHasProducts = createSelector([selectProductsCount], (count) => count > 0);
...
export const selectActiveFiltersCount = createSelector([selectProductsFilters], (filters) => {
  let count = 0;
...
export const selectPriceRange = createSelector([selectProductsFilters], (filters) => ({
...
export const selectSortOptions = createSelector([selectProductsFilters], (filters) => ({
...
export const selectInStockProducts = createSelector([selectProducts], (products: Product[]) =>
  products.filter((product: Product) => product.stockQuantity > 0),
);

export const selectOutOfStockProducts = createSelector([selectProducts], (products: Product[]) =>
  products.filter((product: Product) => product.stockQuantity === 0),
);

export const selectProductsOnSale = createSelector([selectProducts], (products: Product[]) =>
  products.filter(
    (product: Product) => product.discountPrice && product.discountPrice < product.price,
  ),
);

export const selectNewProducts = createSelector([selectProducts], (products: Product[]) => {
  const thirtyDaysAgo = new Date();
  thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
  return products.filter((product: Product) => new Date(product.createdAt) > thirtyDaysAgo);
});

export const selectHighRatedProducts = createSelector([selectProducts], (products: Product[]) =>
  products.filter((product: Product) => product.rating >= 4.0),
);

// Price range selectors
export const selectProductsPriceRange = createSelector([selectProducts], (products: Product[]) => {
  if (products.length === 0) {
    return { min: 0, max: 0 };
  }

  const prices = products.map((product: Product) => product.discountPrice || product.price);
  return {
    min: Math.min(...prices),
    max: Math.max(...prices),
  };
...
export const selectAverageProductPrice = createSelector([selectProducts], (products: Product[]) => {
...
export const selectAnyLoading = createSelector(
  [selectProductsLoading, selectProductDetailsLoading, selectRelatedProductsLoading],
...
export const selectProductBreadcrumb = createSelector([selectProductDetails], (product) => {
  if (!product) return [];
...
export const selectProductsPageTitle = createSelector(
  ...
  (filters, pagination) => {
    ...
    const page = pagination.currentPage > 1 ? ` - Page ${pagination.currentPage}` : "";
...
export const selectProductPageTitle = createSelector([selectProductDetails], (product) => {
  if (!product) return "Product Details";
  return `${product.name} - ${product.brand}`;
});

export const selectProductPageDescription = createSelector([selectProductDetails], (product) => {
  if (!product) return "";
